      ...
      <div class="el-userlist-top">
        ...
        <el-row :gutter="20">
          <el-col :span="2" :offset="6">
            <div class="grid-content bg-purple" @click="addVisible">
              添加用户
            </div>
          </el-col>
        </el-row>
          ...
          <el-table-column prop="mg_state" label="状态" width="170">
            <template slot-scope="scope">
              ...
              <el-switch
                ...
                @change="usersChange(scope.row.id, scope.row.mg_state)"
              >
              </el-switch>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="240">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                @click="set_userRole(scope.row.id)"
              ></el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <div class="newjob" v-if="isNew">
          ...
          >
            ...
            <el-form-item size="large">
              <el-button type="primary" @click="set_roleBtn_ok()"
                >确定</el-button
              >
              <el-button @click="set_roleBtn_cancel()">取消</el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>
...
import { set_role, rights_res } from "./../assets/js/request";
export default {
  props: {},
  data() {
    return {
  ...
  methods: {
    ...
    set_roleBtn_ok() {
      this.isNew = false;
      set_role(this.scopRowId, this.newjob.id - 0).then((res) => {
    ...
    edit(editForm) {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return;
        usersEdit_res(editForm.id, editForm.mobile, editForm.email).then(
          (res) => {
            if (res.meta.status !== 200) {
    ...
    //点击分配权限按钮
    set_userRole(scopRowId) {
      this.scopRowId = scopRowId;
      rights_res().then((res) => {
